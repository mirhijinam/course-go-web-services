...
func printWithFiles(out io.Writer, fullpath, fullpref string) error {
	...
	for index, entity := range dirContent {
		...
		err := error(nil)
		if entity.IsDir() {
			err = printWithFiles(out, fullpath+"/"+entity.Name(), fullprefArg)
		}
		if err != nil {
			return err
		}
...
func printWithoutFiles(out io.Writer, fullpath, fullpref string) error {
	dirContent, err := os.ReadDir(fullpath)
	if err != nil {
		return err
	}
	...
	for _, dir := range dirSet {
		...
		err := printWithoutFiles(out, fullpath+"/"+dir.name, fullprefArg)
		if err != nil {
			return err
		}
	}
